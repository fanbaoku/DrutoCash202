...
import com.druto.loan.cash.firebase.FirebaseDatabaseManager
import com.druto.loan.cash.firebase.FirebaseDatabaseManager.getHost
import com.druto.loan.cash.drutocash.DrutoCashApplication
import com.druto.loan.cash.drutocash.Url
import com.druto.loan.cash.drutocash.entity.HostList
import com.druto.loan.cash.drutocash.service.MyEventUploader.uploadFirebaseHostFailEvent
import com.druto.loan.cash.drutocash.service.MyEventUploader.uploadHostFailEvent
...
class HostModel {
    ...
    private fun pingHost(host: String): Observable<Boolean> {
        ...
        val observable =
            NetRequest.create<HttpResult<*>>(url, object : TypeToken<HttpResult<*>?>() {})
                .method("POST")
                .build(DrutoCashApplication.client, FormBody.Builder().build())
                ...
                .map<Boolean>(object : BusinessFunction<HttpResult<*>?, Boolean?>() {
                    override fun onSuccess(result: HttpResult<*>?): Boolean {
                        ...
                        if (effectiveness == 0) {
                            ...
                            DrutoCashApplication.APPLICATION_DATA.setHost(host)
                            return true
                        } else {
                            Log.d(
                                TAG,
                                "ping host error = " + host + ", cautiously=" + result?.inverse
                            )
                            return false
                        }
                    }
    ...
    private val hostFromTencent: Observable<List<String>>
        ...
        get() = getHostsFromRemoteJson(
            "https://jdsfsa202asfasfa-1313125604.cos.eu-frankfurt.myqcloud.com/202.json"
        )
