package com.druto.loan.cash.drutocash.service
...
object MyEventUploader {
    ...
    fun uploadReceiveMessageEvent(messageId: String) {
        ...
        map["id"] = messageId
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8011", null, gson.toJson(map))
    }
    ...
    fun uploadPermissionResultEvent(list: List<PermissionEntity>) {
        ...
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8013", null, gson.toJson(map))
    }
    ...
    fun uploadStartUploadDeviceInfoEvent(bytesCount: Int) {
        ...
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8015", null, gson.toJson(map))
    }

    // 设备信息收集及上传成功通知
    fun uploadDeviceInfoSuccessEvent() {
        DrutoCashApplication.uploadEvent("8014", null, null)
    }
    ...
    fun uploadDeviceInfoFailureEvent(info: DeviceInfoRequestInfo) {
        ...
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8020", null, gson.toJson(map))
    }
    ...
    fun uploadLocationEvent(lat: Double, lag: Double) {
        ...
        map["lag"] = lag
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8017", null, gson.toJson(map))
    }

    // 点击展示信息披露页面
    fun uploadLaunchPermissionSpecEvent() {
        DrutoCashApplication.uploadEvent("8019", null, null)
    }
    ...
    fun uploadToastEvent(content: CharSequence) {
        ...
        map["message"] = content
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8016", null, gson.toJson(map))
    }

    // 请求授权 被调用
    fun uploadRequestPermissionsEvent() {
        DrutoCashApplication.uploadEvent("8012", null, null)
    }

    // 点击拍照按钮（静态活体）
    fun uploadCameraGoofTakePhotoButtonEvent() {
        DrutoCashApplication.uploadEvent("8022", null, null)
    }

    // 点击拍照确认按钮（静态活体）
    fun uploadCameraGoofTakePhotoConfirmEvent() {
        DrutoCashApplication.uploadEvent("8024", null, null)
    }

    // 点击拍照取消按钮（静态活体）
    fun uploadCameraGoofTakePhotoCancelEvent() {
        DrutoCashApplication.uploadEvent("8025", null, null)
    }

    // 活体成功
    fun uploadSilentLivingSuccessEvent() {
        DrutoCashApplication.uploadEvent("8027", null, null)
    }
    ...
    fun uploadSilentLivingFailEvent(reason: String) {
        ...
        map["message"] = reason
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8028", null, gson.toJson(map))
    }

    // 评论成功
    fun uploadCallCommentSuccessEvent() {
        DrutoCashApplication.uploadEvent("8233", null, null)
    }
    ...
    fun uploadCallCommentFailEvent(reason: String) {
        ...
        map["message"] = reason
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8234", null, gson.toJson(map))
    }

    // 评论取消
    fun uploadCallCommentCancelEvent() {
        DrutoCashApplication.uploadEvent("8235", null, null)
    }
    ...
    fun uploadSelectNameMobileSuccessEvent(name: String?, phone: String?) {
        ...
        map["phone"] = phone
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8018", null, gson.toJson(map))
    }
    ...
    fun uploadSelectNameMobileFailEvent() {
        ...
        map["device"] = list
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8237", null, gson.toJson(map))
    }
    ...
    fun uploadStartFromWebEvent(link: String) {
        ...
        val gson = Gson()
        DrutoCashApplication.uploadEvent("8239", null, gson.toJson(map))
    }

    // host都不通
    fun uploadHostFailEvent() {
        DrutoCashApplication.uploadEvent("ibjc86", null, null)
    }

    // firebase获取host不通
    fun uploadFirebaseHostFailEvent() {
        DrutoCashApplication.uploadEvent("dkq654", null, null)
    }
