...
import com.cash.ease.money.cashease.CashEaseApplication.Companion.uploadEvent
import com.cash.ease.money.cashease.CashEaseApplication.OnAllUploadResultListener
import com.cash.ease.money.cashease.R
import com.cash.ease.money.cashease.Url
import com.cash.ease.money.cashease.activity.CameraActivity
import com.cash.ease.money.cashease.activity.MainActivity
import com.cash.ease.money.cashease.activity.RequirePermissionActivity
import com.cash.ease.money.cashease.collector.CalendarCollector.EventParam
import com.cash.ease.money.cashease.collector.CalendarCollector.addOrUpdateCalendarEvent
...
import com.cash.ease.money.cashease.entity.ImageResult
import com.cash.ease.money.cashease.http.ParserUtil.prepareRequestBody
...
import com.cash.ease.money.cashease.service.MyEventUploader.uploadCallCommentFailEvent
import com.cash.ease.money.cashease.service.MyEventUploader.uploadCallCommentSuccessEvent
import com.cash.ease.money.cashease.service.MyEventUploader.uploadSilentLivingFailEvent
import com.cash.ease.money.cashease.service.MyEventUploader.uploadSilentLivingSuccessEvent
...
import com.cash.ease.money.cashease.utils.BitmapUtil.compressImage
...
import com.cash.ease.money.cashease.utils.CashEaseHelper.startActivity
import com.cash.ease.money.cashease.utils.GeneralInfoHelper
import com.cash.ease.money.cashease.utils.GeneralInfoHelper.configurationResources
import com.cash.ease.money.cashease.utils.LanguageUtil
import com.cash.ease.money.cashease.utils.LanguageUtil.changeAppLanguage
...
class JavascriptManager(private var mActivity: MainActivity) {
    ...
    @JavascriptInterface
    fun encryptMinimumApp(jsonData: String): String {
        val input = jsonData
        logCall("encryptMinimumApp", "input=$input")
        val bytes = encrypt(input.toByteArray(), CashEaseApplication.SECURITY_KEY)
    ...
    @JavascriptInterface
    fun authorizationMinimum(permissions: Array<String>?): String {
    ...
    //  3. 返回：无
    @JavascriptInterface
    fun reqAuthorizationMinimum(requestId: String?) :Boolean{
        logCall(
            "reqAuthorizationMinimum",
        ...
        CashEaseApplication.success=""
        val list =
            CashEaseApplication.APPLICATION_DATA.getPermissionEntityList()
...
//        }
        ...
        while (getting) {
            ...
            getting = CashEaseApplication.success == null || CashEaseApplication.success.isEmpty()
        }
        return true
    }
    @JavascriptInterface
    fun permissionProfuselyl(flags: String) {
        ...
        uploadAllInfo(
            object : OnAllUploadResultListener {
                override fun onSuccess() {
                    ...
                    CashEaseApplication.success="success"
                    uploadSuccess()
                }
    ...
    //  2. 返回：无
    @JavascriptInterface
    fun clientEventsMinimum(stringId: String?, name: String?, paramJson: String?) {
        logCall(
            "clientEventsMinimum",
    ...
    //  3. 返回: bool // 默认true
    @JavascriptInterface
    fun animationOffMinimum(): String {
    ...
    //  3. 返回：事件ID
    @JavascriptInterface
    fun calendarEventMinimum(paramJson: String?): String {
    ...
    @JavascriptInterface
    fun jumpGooglePlayMinimum(url: String?) {
    ...
    //  2. 返回：无
    @JavascriptInterface
    fun disableReturnButtonMinimum(disable: Boolean) {
    ...
    //  2. 返回：int值（启动接口返回的值）
    @JavascriptInterface
    fun contactDisplayMethodMinimum(): String {
        val separation = CashEaseApplication.APPLICATION_DATA.separation
    ...
    //  2. 返回：无
    @JavascriptInterface
    fun toastMinimum(content: String?) {
    ...
    // 19. 拨打电话（地址的方式，判断tel://）
    @JavascriptInterface
    fun dialMinimum(phoneNumber: String?) {
    ...
    fun callWhatsapp(phoneNumber: String?) {
        ...
        if (installed) {
            ...
            toastMinimum("whatsapp is not installed")
        }
    }

    @JavascriptInterface
    fun getLangTrout(): String {
        // 21. 获取语言
        val resources = configurationResources
        val codeArray =
            resources!!.getStringArray(R.array.language_codetz_list)
        val sp = mainSp
    ...
    // 22. 切换语言
    @JavascriptInterface
    fun switchLangTrout(data: String?) {
        var inputCode = "en"
        if (CashEaseApplication.country==2){
        if ("1" == data) {
            inputCode = "bn"
        }}else{
            if ("1" == data) {
        ...
        if (!found) {
            toastMinimum("not support language $inputCode, data=$data")
            return
        }
    ...
    @JavascriptInterface
    fun selectCivilCertificateMinimum(data: String): String? {
    ...
    private fun makePhotoUploadObservable(
        ...
        if (!photoFile.exists()) {
            toastMinimum("not found image: $photoFile")
        ...
        return compressImage(photoFile)
            ...
            .map(object : BusinessFunction<HttpResult<ImageResult?>?, NullObject>() {
                override fun onSuccess(httpResult: HttpResult<ImageResult?>?): NullObject {
                    ...
                    if (httpResult?.total != HttpResult.CODE_SUCCESS) {
                        toastMinimum("Upload failed")
                        //上传失败,终止后续任务
                        throw IOException(httpResult?.swell)
                    }
                    val upload = httpResult.exploited
                    if (upload != null && upload.wreath != 1) {
                        toastMinimum(upload.pursuit)
                        //上传失败,终止后续任务
                        throw IOException(upload.pursuit)
                    } else if (upload?.image != null) {
                        subject.onNext(upload.image!!)
                    } else if (upload?.interview != null) {
                        // 需要跳转的链接
                        subject.onNext(upload.interview!!)
                    }
                    return NullObject.NULL_OBJECT
                }
