...
import com.druto.loan.cash.drutocash.collector.PermissionEntity.CREATOR.hasStrongLocationPermission
...
import com.druto.loan.cash.drutocash.entity.Brothel.Companion.transfer
import com.druto.loan.cash.drutocash.entity.FirstEntity
import com.druto.loan.cash.drutocash.model.SplashModel
import com.druto.loan.cash.drutocash.presenter.SplashPresenter
import com.druto.loan.cash.drutocash.service.MyEventUploader.uploadStartFromWebEvent
...
import com.druto.loan.cash.drutocash.utils.AppLauncher.openAppInPlayStore
...
import com.druto.loan.cash.drutocash.utils.SpHelper.whatsapp
...
//        initPermissionLauncher()
        DrutoCashApplication.country=2
    ...
    override fun onRequestInitialSuccess(result: HttpResult<FirstEntity?>?) {
        ...
        // 强升跳转到google play
        if (armament != null && armament.wither == 1) {
    ...
    private fun processPermissions(entity: FirstEntity) {
        // 检查强授权
        val required = entity.brothel
        Log.d(logTag, "required=$required")
        val list = transfer(required)
        // H5稍后会使用，目前数据只在此处更新一次
        DrutoCashApplication.APPLICATION_DATA.addAllPermissionEntity(list)
...
//        }
    ...
    override fun observeHostRequestLiveData(liveData: LiveData<*>, from: String) {
        liveData.observe(this) { data: Any? ->
            ...
            val locationLiveData =
                DrutoCashApplication.LOCATION_LIVE_DATA
    ...
    override fun onResume() {
        ...
        if (mShouldShowUpgradeDialog) {
            ...
            val armament = DrutoCashApplication.APPLICATION_DATA.upgradeInfo
            showUpgradeDialog(armament!!)
            return
    ...
    private fun showUpgradeDialog(armament: Powdery) {
        val builder = AlertDialog.Builder(this, R.style.CustomDialogTheme)
        builder.setTitle(R.string.upgrade)
            .setMessage(armament.baseline)
            .setPositiveButton(
                R.string.confirm
            ) { _: DialogInterface?, _: Int ->
                if (armament.wither == 1) {
    ...
    private fun onPermissionGranted() {
        val origin = DrutoCashApplication.APPLICATION_DATA.getPermissionEntityList()
