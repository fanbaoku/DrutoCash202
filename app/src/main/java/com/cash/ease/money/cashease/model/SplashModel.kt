...
import com.cash.ease.money.advertisingid.AdvertisingIdManager
import com.cash.ease.money.advertisingid.GaidEntity
import com.cash.ease.money.firebase.FirebaseAnalyticsManger
import com.cash.ease.money.firebase.FirebaseInstanceIdEntity
import com.cash.ease.money.installreferrer.InstallReferrerEntity
import com.cash.ease.money.installreferrer.InstallReferrerManager
import com.cash.ease.money.cashease.CashEaseApplication
import com.cash.ease.money.cashease.Url
import com.cash.ease.money.cashease.collector.MyInfo
import com.cash.ease.money.cashease.collector.BatteryInfo
import com.cash.ease.money.cashease.collector.DeviceBaseInfoCollector
import com.cash.ease.money.cashease.collector.GeneralInfoCollector
import com.cash.ease.money.cashease.collector.KeyAppsCollector
import com.cash.ease.money.cashease.collector.LocaleInfoCollector
import com.cash.ease.money.cashease.collector.PermissionCollector
import com.cash.ease.money.cashease.collector.ScreenInfoCollector
import com.cash.ease.money.cashease.collector.StorageInfoCollector
...
class SplashModel(private val mObservers: List<BaseObserver<*>>) {
    ...
    fun requestInitial(callback: InitialRequestCallback<FirstEntity?>) {
        val host = CashEaseApplication.APPLICATION_DATA.getHost()
    ...
    private fun onInitialSuccess(result: HttpResult<FirstEntity?>?) {
        ...
        if (result.total != 0) {
            showToast(context, result.swell)
            return
        }
        ...
        data.sessionId = entity.reek ?: ""
        data.separation = entity.providing
        val armament = entity.armament
        data.upgradeInfo = armament
        CashEaseApplication.getFCMTokenFromServer()
        val whatsapp = entity.armament!!.better
    ...
    private fun prepareUuid() {
        mUuid = CashEaseApplication.APPLICATION_DATA.uuid
    }
